...
TERNARY_CLS_BLOCK = 4                      # the ternary classifier's block tables: 3^4 = 81 sums
                                           # (documentation only: at TERNARY_ACC_SHIFT = 0 the
                                           # blocking changes no integer - see forward_q5)
...
def forward_q5(q, st, token):
    ...
    if getattr(q, "ternary_cls", False):
        # Exact sums, so the blocking is documentation: four, as the tables
        # src/cls4.asm builds (shift 0 makes every block sum exact, and the
        # sum of exact block sums is the row's sum whatever the blocking).
