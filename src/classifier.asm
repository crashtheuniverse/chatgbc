; The classifier: the one WRAM bank of tables it works from, and the two older
; kernels that share it.
;
; Three classifiers have lived here, chosen by the exporter from the
; checkpoint. The ternary one - blocks of four inputs, output-major, the sum in
; a register pair and the argmax fused - is the one that ships, and it is its
; own file: src/cls4.asm. This file keeps what the others still need:
;
;   CLS_BINARY  blocks of eight one-bit inputs on two 256-entry planes,
;               input-major into stored int16 sums, then Cls_Argmax rescans.
;   int8        the v0.4 kernel: every input's product table copied into the
;               bank, output-major with the sum in de (CLS_MAC, 24 cycles a
;               MAC). The exported weight bytes do the addressing - each is
;               the low byte of its entry's address, the input's table base
;               folded in at export, and the high byte advances every four
;               inputs in b. The raw sums are comparable because the 8-bit
;               classifier has one scale for every row; export asserts that.
;
; Both of these keep the sums or the best in WRAM and take the no-repeat
; retry as a rescan over wBlocked; cls4.asm keeps a top-two list instead.

INCLUDE "hardware.inc"
INCLUDE "chatgbc.inc"
INCLUDE "model.inc"

; One WRAM bank (CLS_TBL_BANK, chatgbc.inc) holds every table the kernel in
; force builds: the block-4 tables' sixteen pages, the one-bit planes above
; the sums, or every input's product table at wClsTbl + j*64.
...
hClsPart:  db                       ; the part being accumulated
